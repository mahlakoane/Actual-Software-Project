...
void Maze::ReadEntityFromFile()
{
    ...
    if (!inputFile)
    {
        std::cerr << "File could not be opened"; ///Should be an exception
    }
...
std::vector<mazeWall> Maze::ReturnEntityVec() const
{
    return LogicMaze;
}


std::vector<mazeWall> Maze::HitBox() const
{
  return LogicMaze;
}

void Maze::DestroyEntity()
{
